  ...
  <div>
    <div class="container">
      ...
      <ul class="step-list text-coffee text-bold">
        <li
          class="col-sm-12 col-lg-3 pt-2 mt-3"
      ...
      <div class="col-md-12 col-lg-9 mx-auto" v-if="step === 1">
          ...
          <tbody>
            <tr v-for="item in cart.carts" :key="item.id">
              <td class="phone-d-none" style="width: 20%;">
                <div
                  style="height: 80px; background-size: cover; background-position: center center;"
                ...
                <div class="input-group input-group-sm justify-content-center">
                  <div class="input-group-prepend">
                    <button
                      ...
                      type="button"
                      @click.prevent="changeQty(item.id, item.product.id, item.qty, 0)"
                  ...
                  <input
                    type="text"
                    style="width: 25px;"
                    :style="{ opacity: isChangingQty ? '0.5' : '1' }"
                    aria-label="Example text with button addon"
                    aria-describedby="button-addon1"
                    @blur="changeQty(item.id, item.product.id, item.qty, 3)"
                    v-model="item.qty"
                  />
                  <div class="input-group-append">
                    <button
                      class="btn btn-coffee"
                      type="button"
                      @click.prevent="changeQty(item.id, item.product.id, item.qty, 1)"
                      :disabled="isChangingQty"
                    >
                      +
                    </button>
        ...
        <template v-if="orderTotal == 0">
          <div class="col-md-9 mx-auto mt-2">
            <div
              class="noProductImg"
              style="background-image: url(https://i.imgur.com/vS4QRri.jpg);"
            >
              <div class="noProductOut">
                <div class="noProductIn">
                  <h5 class="pt-3">Opps .....</h5>
                  <span class="d-block pt-3">您目前沒有產品在購物車</span>
                  <button class="btn btn-coffee mt-3 hvr-rotate" @click.prevent="goProductList()">
                    點我去選購產品
                  </button>
                </div>
              </div>
      ...
      <div v-if="step === 3">
        ...
        <div v-if="checkPoint === true">
          <div class="col-md-9 mx-auto text-center" id="accordion">
            <div
              class="noProductImg"
              style="background-image: url(https://i.imgur.com/dfQksCq.jpg);"
            >
              <div class="noProductOut">
                <div class="noProductIn">
                  <h5 class="pt-3">感謝您的購買</h5>
                  <span class="d-block pt-3">祝您有個美好愉快的一天</span>
                  <button class="btn btn-coffee mt-3" @click.prevent="goProductList()">
                    點我回首頁
                  </button>
                </div>
              </div>
            ...
            <div class="card" @click.prevent="point = 1">
              ...
              >
                <h6
                  class="text-center"
                  @click.prevent="point = 1"
                  :class="{ 'text-white , active': point === 1 }"
                >
                  購買訂單
                </h6>
              </button>
            ...
            <div class="card" @click.prevent="point = 2">
              <button
                class="btn description-btn collapsed"
                data-toggle="collapse"
              ...
              >
                <h6 :class="{ 'text-white , active': point === 2 }" @click.prevent="point = 2">
                  個人資料
                </h6>
              </button>
              <div
...
export default {
  ...
  methods: {
    ...
    changeQty(cartId, productId, num, isAdd) {
      this.isChangingQty = true;
      this.newNum = Number(num);
      if (isAdd === 1) {
        this.newNum += 1;
      } else if (isAdd === 0) {
        this.newNum -= 1;
      ...
      const vm = this;
      const api = `https://vue3-course-api.hexschool.io/v2/api/${process.env.VUE_APP_NAME}/cart/${cartId}`;
      const newCart = {
        product_id: productId,
        qty: this.newNum,
      };
      vm.$http.put(api, { data: newCart }).then(() => {
        vm.getCartItem();
